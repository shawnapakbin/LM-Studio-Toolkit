...
/**
 * Cached Blender version detected from the connected instance.
 * Set on first successful call to getBlenderVersionCached().
 */
...
/**
 * Detects the Blender version by executing `bpy.app.version` via the addon.
 * Caches the result after the first successful call to avoid repeated queries.
 *
 * @returns A tuple [major, minor, patch] representing the Blender version.
 */
export async function getBlenderVersionCached(
  host: string,
  port: number,
  timeoutMs: number,
  ...
  const code = "import bpy\nresult = list(bpy.app.version)";
  const response = await sendCodeToAddon(host, port, timeoutMs, code, true);

  if (response.status === "error") {
    throw new Error(
      `Failed to detect Blender version: ${(response.message as string) || "Unknown error"}`,
    );
  }
...
/**
 * Resets the cached Blender version. Intended for testing purposes.
 */
...
/**
 * Lazily applies the Blender 5.x API compatibility layer to Python code.
 *
 * Attempts to retrieve the cached Blender version; if the version is >= 5.0.0,
 * applies API compatibility mappings (remapping deprecated calls to their 5.x equivalents).
 * If version detection fails (addon not connected, first call before version is cached, etc.),
 * the code passes through unchanged.
 *
 * Standard code that does not use any known-changed APIs passes through
 * character-for-character regardless of the detected version.
 */
async function applyCompatIfNeeded(code: string, config: BlenderBridgeConfig): Promise<string> {
  try {
    const version = await getBlenderVersionCached(
    ...
    if (version[0] >= 5) {
      return applyCompatibilityLayer(code, version);
    }
  } catch {
    // Version detection failed (addon not connected yet, first call, etc.)
 ...
 *
 * When a Blender 5.x version is detected (via cached version query), applies
 * API compatibility mappings to remap deprecated calls to their 5.x equivalents.
 * Standard code that does not use deprecated APIs passes through unchanged.
 */
export function createAddonExecuteCodeDelegate(config: BlenderBridgeConfig): ExecuteBlenderCodeFn {
  return async (pythonCode: string): Promise<string> => {
    // Apply compatibility layer if Blender version is 5.x+
    const codeToSend = await applyCompatIfNeeded(pythonCode, config);
 ...
 *
 * Applies Blender 5.x API compatibility mappings to the generated code when
 * the detected version is >= 5.0.0. Standard code passes through unchanged.
 */
export function createAddonCallToolDelegate(
  ...
  return async (toolName: string, args: Record<string, unknown>): Promise<CallToolContent[]> => {
    ...
    // Apply compatibility layer if Blender version is 5.x+
    const codeToSend = await applyCompatIfNeeded(code, config);
