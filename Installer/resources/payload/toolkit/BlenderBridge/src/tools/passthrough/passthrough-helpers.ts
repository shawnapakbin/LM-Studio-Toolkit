...
export function normalizeCodeParam(rawCode: unknown): string | null {
  ...
  if (typeof rawCode === "object") {
    ...
    // Check fields in priority order: python, code, text, script, command, result
    // "result" is checked last because LLMs sometimes echo back a previous tool result
    // as input — if the result value looks like valid Python code, we can recover.
    for (const field of ["python", "code", "text", "script", "command", "result"]) {
      const val = obj[field];
      if (typeof val === "string" && val.length > 0 && looksLikePythonCode(val)) {
        return val;
      }
    }
    // Second pass: accept any string field value that's non-trivial (>10 chars)
    // to recover from unusual field names
    for (const field of ["python", "code", "text", "script", "command"]) {
      const val = obj[field];
...
/**
 * Heuristic check for whether a string looks like Python code rather than a plain value.
 * Returns true if the string contains common Python patterns (imports, assignments, keywords, etc.)
 */
function looksLikePythonCode(value: string): boolean {
  // Short values like "Layout", "True", "None" are unlikely to be code
  if (value.length < 10) return false;
  // Check for common Python code indicators
  const pythonPatterns =
    /\b(import|from|def|class|print|bpy|result\s*=|for|while|if|return|with)\b|[=\(\)\[\]:]/;
  return pythonPatterns.test(value);
}
 ...
 * Designed to help LLMs break out of retry loops by providing explicit correct usage examples.
 */
export function buildDiagnosticError(rawValue: unknown, paramName: string): string {
  ...
  // Detect the "echoing back results" pattern where the LLM sends a previous tool result as input
  const isResultEcho =
  ...
  const echoWarning = isResultEcho
    ? "It appears you sent a previous tool result as input instead of Python code. "
    : "";
  ...
  return (
    ...
    `Correct example: {"code": "import bpy\\nresult = bpy.context.workspace.name"}`
  );
}
