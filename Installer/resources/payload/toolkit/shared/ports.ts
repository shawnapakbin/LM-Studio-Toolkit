...
export const TOOL_PORTS: Record<string, number> = {
  ...
  fileeditor: Number(process.env.FILEEDITOR_PORT ?? 3012),
  packagemanager: Number(process.env.PACKAGEMANAGER_PORT ?? 3013),
  slashcommands: Number(process.env.SLASHCOMMANDS_PORT ?? 3345),
  blenderbridge: Number(process.env.BLENDERBRIDGE_PORT ?? 3346),
};
