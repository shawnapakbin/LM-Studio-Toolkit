...
/**
 * Session-based approval controller for tools that require explicit user consent.
 *
 * Supports:
 *  - One-time approval tokens
 *  - Session-scoped grants (approve once for the whole session)
 *  - Environment variable bypass for CI/automation
 *  - AskUser interview integration for interactive approval flows
 */

import crypto from "crypto";
import { ErrorCode, createErrorResponse, type ToolResponse } from "./types";

const DEFAULT_APPROVAL_TOKEN_TTL_MS = 15 * 60 * 1000; // 15 minutes
...
export class SessionApprovalController {
  ...
  private readonly pendingApprovalTokens = new Map<string, PendingToken>();
  ...
  private async createApprovalInterview(
    action: string,
    details: string,
  ): Promise<string | null> {
  ...
  requestApproval(
    ...
    return {
      ...
      errorMessage:
        "User approval is required before this operation can proceed.",
      data: {
  ...
  async ensureApproved(
    ...
    // Check one-time approval token
    if (input.approvalToken) {
      const result = this.redeemApprovalToken(
        input.approvalToken,
        input.action,
        sessionKey,
      );
    ...
    // Check interview-based approval
    if (input.approvalInterviewId) {
      ...
      const responses =
        (body?.responses as Array<Record<string, unknown>>) ||
        ((body?.data as Record<string, unknown>)?.responses as Array<Record<string, unknown>>) ||
        [];
      ...
      if (
        status === "answered" &&
        (approval?.value === "allow_once" || approval?.value === true)
      ) {
        return { ok: true };
      }
      ...
      if (
        status === "answered" &&
        (approval?.value === "allow_session" ||
          approval?.value === "allow_in_session")
      ) {
        if (sessionKey) {
    ...
    // No prior approval — create an interview form for the user
    const interviewId = await this.createApprovalInterview(
      input.action,
      input.details,
    );
    return {
